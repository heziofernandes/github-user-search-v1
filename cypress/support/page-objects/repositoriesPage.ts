...
export class RepositoriesPage {
  ...
  async returnRepositories(repo: string) {
    cy.get('[data-testid="main-title"]').should('be.visible').invoke('text').then(messageNotFound => {
      expect(messageNotFound).to.equal('Look who we found 🔥')
    })
    cy.get('[data-testid^="reponame-"]').invoke('text').then((text) => {
  ...
  async userNotFound() {
    cy.get('[data-testid="user-not-found"]').invoke('text').then(messageNotFound => {
  ...
  async ReturnUserWithoutRepository(){
    ...
    cy.get('[data-testid="repo-count"]').invoke('text').then(repository=>{
